...
fun ExploreResult.matches(category: ExploreCategory): Boolean = when (category) {
    ...
    ExploreCategory.PEOPLE -> target is CollaborationTarget.Organization ||
...
object TeamSpaceProjection {
    fun build(
        ...
        val members = memberships.asSequence()
            ...
            .sortedWith(compareByDescending<TeamMemberSummary> { it.role == TeamRole.MAINTAINER }.thenBy { it.user.displayName })
            .toList()
...
object RepositoryOverviewProjection {
    fun build(
        ...
        when (repository.ownerType) {
            ...
            OwnerType.ORGANIZATION -> participantUserIds += orgMemberships
...
fun AppNotification.matches(view: InboxPrimaryView): Boolean = when (view) {
    ...
    InboxPrimaryView.MENTIONS_OR_ACTION -> status != NotificationStatus.ARCHIVED &&
...
data class SavedGroupItems(
    val group: SavedGroup,
    val targets: List<CollaborationTarget>
)
...
object SavedProjection {
    ...
    private fun groupFor(target: CollaborationTarget): SavedGroup = when (target) {
        ...
        is CollaborationTarget.UserProfile -> SavedGroup.PEOPLE
    }
}
