                        ...
                        if (experienceDao.countUnsynced(mutation.entityType, mutation.entityId) > 0) {
                            ...
                            experienceDao.upsertSyncMetadata(
                                SyncMetadata(
                                    ...
                                    status = SyncState.SYNCED,
                                    updatedAt = System.currentTimeMillis(),
                                    deletedAt = if (mutation.operation ==
                                        SyncOperation.DELETE
                                    ) {
                                        System.currentTimeMillis()
                                    } else {
        ...
        return runCatching {
            when (mutation.entityType) {
                SyncEntityType.REPOSITORY -> {
                    ...
                    if (entity.ownerType != com.example.data.model.OwnerType.USER &&
                        entity.ownerType != com.example.data.model.OwnerType.ORGANIZATION
                    ) {
                        return false
                    }
                ...
                SyncEntityType.ISSUE -> {
                    ...
                    if (entity.id != mutation.entityId || governanceDao.getRepositoryByIdOnce(
                            entity.repoId,
                        ) == null
                    ) {
                        return false
