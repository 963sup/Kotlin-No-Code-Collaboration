...
replace_once(
    ...
    '''        val updatedIssue = issue.copy(\n            status = newStatus,\n            updatedAt = now,\n            closedAt = if (newStatus == IssueStatus.CLOSED) now else null,\n            closedByUserId = if (newStatus == IssueStatus.CLOSED) actor.id else null,\n            closedByDisplayName = if (newStatus == IssueStatus.CLOSED) actor.displayName else null\n        )''',
...
replace_once(
    ...
    '''    suspend fun updateIssuePlan(\n        actorId: String,\n        issueId: String,\n        sortOrder: Int,\n        plannedStartAt: Long?,\n        plannedEndAt: Long?,\n        wbsWeight: Double,\n        progressPercent: Int\n    ): OperationResult {\n        val actor = dao.getUserByIdOnce(actorId) ?: return OperationResult(false, "找不到操作用戶")\n        val issue = dao.getIssueByIdOnce(issueId) ?: return OperationResult(false, "找不到任務")\n        val repo = dao.getRepositoryByIdOnce(issue.repoId) ?: return OperationResult(false, "找不到儲存庫")\n        val evaluation = evaluate(actor, repo, null, GovernanceAction.ASSIGN_ISSUE)\n        if (evaluation.verdict != PolicyVerdict.ALLOWED) {\n            writeAudit(actor, repo, GovernanceAction.ASSIGN_ISSUE, evaluation)\n            return OperationResult(false, evaluation.finalExplanation)\n        }\n\n        val validationError = IssueHierarchyRules.validatePlan(\n            sortOrder = sortOrder,\n            plannedStartAt = plannedStartAt,\n            plannedEndAt = plannedEndAt,\n            wbsWeight = wbsWeight,\n            progressPercent = progressPercent\n        )\n        if (validationError != null) return OperationResult(false, validationError)\n\n        val hasChildren = dao.getSubIssuesOnce(issue.id).isNotEmpty()\n        val effectiveProgress = when {\n            issue.status == IssueStatus.CLOSED -> 100\n            hasChildren -> issue.progressPercent\n            else -> progressPercent\n        }\n        val now = System.currentTimeMillis()\n        dao.insertIssue(\n            issue.copy(\n                sortOrder = sortOrder,\n                plannedStartAt = plannedStartAt,\n                plannedEndAt = plannedEndAt,\n                wbsWeight = wbsWeight,\n                progressPercent = effectiveProgress,\n                updatedAt = now\n            )\n        )\n        writeAudit(\n            actor = actor,\n            repo = repo,\n            action = GovernanceAction.ASSIGN_ISSUE,\n            evaluation = evaluation,\n            overrideActionName = "UPDATE_ISSUE_PLAN",\n            overrideReasoning = "Updated WBS order, planned dates, weight and leaf progress for #${issue.issueNumber}."\n        )\n        return OperationResult(true, "已更新任務 #${issue.issueNumber} 的 WBS 計畫")\n    }\n\n    suspend fun createDiscussion(\n        actorId: String,'''
)

replace_once(
    "app/src/main/java/com/example/ui/viewmodel/GovernanceViewModel.kt",
    '''    fun createDiscussion(\n        title: String,''',
    '''    fun updateIssuePlan(\n        issueId: String,\n        sortOrder: Int,\n        plannedStartAt: Long?,\n        plannedEndAt: Long?,\n        wbsWeight: Double,\n        progressPercent: Int\n    ) {\n        val actorId = _activeUser.value?.id ?: return\n        viewModelScope.launch {\n            emitResult(\n                repository.updateIssuePlan(\n                    actorId = actorId,\n                    issueId = issueId,\n                    sortOrder = sortOrder,\n                    plannedStartAt = plannedStartAt,\n                    plannedEndAt = plannedEndAt,\n                    wbsWeight = wbsWeight,\n                    progressPercent = progressPercent\n                )\n            )\n        }\n    }\n\n    fun createDiscussion(\n        title: String,'''
)
